    ...
    def _get_page(self, pagerequest: str) -> str:
        ...
        while tries < self._max_retries:
            try:
                ...
                self.logger.info(
                    "Session proxy config is {}".format(self._session.proxies)
                )
                ...
                if not self.pm.has_proxy():
                    ...
                    w = random.uniform(60, 2 * 60)
                    self.logger.info(
                        "Will retry after {} seconds (with the same session).".format(w)
                    )
                    time.sleep(w)
                    continue
            except Timeout as e:
                err = f"Timeout Exception %s while fetching page: %s" % (
                    type(e).__name__,
                    e.args,
                )
                ...
                err = f"Exception %s while fetching page: %s" % (
                    type(e).__name__,
                    e.args,
                )
                self.logger.info(err)
            ...
            tries += 1
            self._session, timeout = self.pm.get_next_proxy(
                num_tries=tries, old_timeout=timeout
            )
