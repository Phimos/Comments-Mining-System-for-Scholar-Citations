...
class _SearchScholarIterator(object):
    ...
    def __next__(self):
        if self._pos < len(self._rows):
            ...
            self._pos += 1
            res = self.pub_parser.get_publication(
                row, PublicationSource.PUBLICATION_SEARCH_SNIPPET
            )
            return res
...
class PublicationParser(object):
    ...
    def _citation_pub(self, __data, publication: Publication):
        ...
        publication["author_pub_id"] = re.findall(
            _CITATIONPUBRE, __data.find("a", class_="gsc_a_at")["data-href"]
        )[0]
    ...
    def fill(self, publication: Publication) -> Publication:
        ...
        if publication["source"] == PublicationSource.AUTHOR_PUBLICATION_ENTRY:
            ...
            if soup.find("a", class_="gsc_vcd_title_link"):
                publication["pub_url"] = soup.find("a", class_="gsc_vcd_title_link")[
                    "href"
                ]
            ...
            cites_year = [
                int(c.get("href")[-4:]) for c in soup.find_all(class_="gsc_vcd_g_a")
            ]
